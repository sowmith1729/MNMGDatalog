...
void benchmark(int argc, char** argv) {
    ...
    double max_buffer_preparation_time_before_join = 0.0,
           max_communication_time_before_join = 0.0;
    double max_buffer_preparation_time_after_join = 0.0,
           max_communication_time_after_join = 0.0;
    double buffer_preparation_time_before_join = 0.0,
           communication_time_before_join = 0.0;
    double buffer_preparation_time_after_join = 0.0,
           communication_time_after_join = 0.0;
    ...
    int* local_data_host =
        parallel_generate(total_rank, rank, total_rows, total_columns,
                          rand_range, &row_size, &temp_file_io_time);
    ...
    Entity* hash_table = get_hash_table(grid_size, block_size, input_relation,
                                        input_relation_size, &hash_table_rows,
    ...
    Entity* join_result = get_join(
        ...
        reverse_relation_size, &join_result_size, &temp_join_time);
    ...
    if (total_rank == 1) {
        ...
        communication_time_temp = 0.0;
        buffer_memory_clear_time_temp = 0.0;
        distributed_join_result = get_split_relation(
    ...
    if (job_run == 0) {
        ...
        parallel_write(rank, total_rank, output_file_name,
                       ...
                       distributed_join_result_size, &temp_file_write_time);
    ...
    total_time =
        initialization_time + hashtable_build_time + join_time +
    ...
    if (total_time == max_total_time) {
        ...
        max_buffer_preparation_time_before_join =
            buffer_preparation_time_before_join;
        max_communication_time_before_join = communication_time_before_join;
        max_buffer_preparation_time_after_join =
            buffer_preparation_time_after_join;
        ...
        printf(
            "# Input,# Process,# Iterations,# Output,Total Time,Join,Buffer "
            "preparation (data distribution),Communication (data "
            "distribution),Buffer preparation (join result),Communication "
            "(join "
            "result),Deduplication,Clear,Copy,Finalization,Initialization,File "
            "I/O,Hashtable\n");
        printf(
            ...
            max_buffer_preparation_time_before_join,
            max_communication_time_before_join,
            max_buffer_preparation_time_after_join,
            max_communication_time_after_join, output.deduplication_time,
            max_clear_time, max_copy_to_host_time, output.finalization_time,
            output.initialization_time, output.fileio_time,
            output.hashtable_build_time);
    }
    MPI_Finalize();
}
...
// make runsinglejoin DATA_FILE=100 NPROCS=4 CUDA_AWARE_MPI=0 METHOD=0
// RAND_RANGE=100 make runsinglejoin DATA_FILE=10 NPROCS=4 CUDA_AWARE_MPI=0
// METHOD=0 RAND_RANGE=100 DATA_FILE>10M, strong scaling, otherwise weak
// scaling; make runsinglejoin DATA_FILE=10000000 NPROCS=4 CUDA_AWARE_MPI=0
// METHOD=0 RAND_RANGE=100000 make runsinglejoin DATA_FILE=20000000 NPROCS=4
// CUDA_AWARE_MPI=0 METHOD=0 RAND_RANGE=1000000

// Exp 1: control output size linearly
// make runsinglejoin DATA_FILE=5000000 NPROCS=1 CUDA_AWARE_MPI=0 METHOD=0
// RAND_RANGE=1000000 mpirun -np 1 ./single_join.out 5000000 0 0 1000000 mpirun
// -np 2 ./single_join.out 5000000 0 0 2000000 mpirun -np 4 ./single_join.out
// 5000000 0 0 4000000 mpirun -np 8 ./single_join.out 5000000 0 0 8000000 make
// runsinglejoin DATA_FILE=5000000 NPROCS=2 CUDA_AWARE_MPI=0 METHOD=0
// RAND_RANGE=1000000 make runsinglejoin DATA_FILE=5000000 NPROCS=1
// CUDA_AWARE_MPI=0 METHOD=0 RAND_RANGE=500000
