...
void benchmark(int argc, char** argv) {
    ...
    cudaMemcpy(edge_temp_device, edge_host, local_count * sizeof(int),
               cudaMemcpyHostToDevice);
    // Ensure edges are bidirectional by adding reverse edges
    int* edge_reverse_temp_device;
    checkCuda(cudaMalloc((void**)&edge_reverse_temp_device,
                         local_count * sizeof(int)));
    end_time = MPI_Wtime();
    elapsed_time = end_time - start_time;
    initialization_time += elapsed_time;
    timer.start_timer();
    get_reverse_ar<<<grid_size, block_size>>>(edge_temp_device, row_size,
                                              edge_reverse_temp_device);
    timer.stop_timer();
    ...
    initialization_time += elapsed_time;
    timer.start_timer();
    create_entity_ar_with_offset<<<grid_size, block_size>>>(edge_temp_device,
                                                            row_size, edge, 0);
    create_entity_ar_with_offset<<<grid_size, block_size>>>(
        edge_reverse_temp_device, row_size, edge, row_size);
    timer.stop_timer();
    kernel_time = timer.get_spent_time();
    initialization_time += kernel_time;
    ...
    Entity* distributed_edge = get_split_relation(
        rank, edge, edge_size, total_columns, total_rank, grid_size, block_size,
        cuda_aware_mpi, &distributed_edge_size, comm_method,
    ...
    initialization_time += elapsed_time;
    timer.start_timer();
    same_key_value_entity_ar<<<grid_size, block_size>>>(distributed_edge,
                                                        cc_size, cc);
    timer.stop_timer();
    kernel_time = timer.get_spent_time();
    initialization_time += kernel_time;
    ...
    cc_size =
        (thrust::unique(thrust::device, cc, cc + cc_size, is_equal_key())) - cc;
    timer.stop_timer();
    kernel_time = timer.get_spent_time();
    deduplication_time += kernel_time;
    ...
    Entity* hash_table = get_hash_table(grid_size, block_size, distributed_edge,
                                        distributed_edge_size, &hash_table_rows,
...
//        "join_result", 0);
        ...
        Entity* distributed_join_result = get_split_relation(
            ...
            &distributed_join_result_size, comm_method,
            &buffer_preparation_time_temp, &communication_time_temp,
            &buffer_memory_clear_time_temp, iterations);
        buffer_preparation_time += buffer_preparation_time_temp;
        communication_time += communication_time_temp;
...
//        distributed_join_result_size, rank, "distributed_join_result
//        deduplicated", 0);
#endif
        ...
        timer.start_timer();
        thrust::merge(thrust::device, cc, cc + cc_size, distributed_join_result,
                      distributed_join_result + distributed_join_result_size,
                      new_cc, set_cmp());
        timer.stop_timer();
        kernel_time = timer.get_spent_time();
        ...
        timer.start_timer();
        t_delta_size =
            thrust::set_difference(thrust::device, new_cc, new_cc + new_cc_size,
                                   cc, cc + cc_size, t_delta_temp, set_cmp()) -
            t_delta_temp;
        timer.stop_timer();
        kernel_time = timer.get_spent_time();
        merge_time += kernel_time;
    ...
    // We are interested only the unique component ID, thus we make the
    // component ID as key and got rid of node
    timer.start_timer();
    ...
    communication_time_temp = 0.0;
    buffer_memory_clear_time_temp = 0.0;
    Entity* cc_distributed = get_split_relation(
        rank, cc, cc_size, total_columns, total_rank, grid_size, block_size,
        cuda_aware_mpi, &cc_distributed_size, comm_method,
        &buffer_preparation_time_temp, &communication_time_temp,
        &buffer_memory_clear_time_temp, iterations);
    buffer_preparation_time += buffer_preparation_time_temp;
    communication_time += communication_time_temp;
    ...
    timer.start_timer();
    thrust::sort(thrust::device, cc_distributed,
                 cc_distributed + cc_distributed_size, set_cmp());
    timer.stop_timer();
    kernel_time = timer.get_spent_time();
    deduplication_time += kernel_time;
    ...
    int* component_ids;
    checkCuda(
        cudaMalloc((void**)&component_ids, cc_distributed_size * sizeof(int)));
    int *unique_component_ids, *component_sizes;
    checkCuda(cudaMalloc((void**)&unique_component_ids,
                         cc_distributed_size * sizeof(int)));
    checkCuda(cudaMalloc((void**)&component_sizes,
                         cc_distributed_size * sizeof(int)));
    end_time = MPI_Wtime();
    elapsed_time = end_time - start_time;
    ...
    timer.start_timer();
    thrust::transform(thrust::device, cc_distributed,
                      cc_distributed + cc_distributed_size, component_ids,
    ...
    long long max_component_size_current_rank = thrust::reduce(
        thrust::device, component_sizes,
        component_sizes + total_unique_component, -1, thrust::maximum<int>());
    timer.stop_timer();
    kernel_time = timer.get_spent_time();
    merge_time += kernel_time;
    ...
    MPI_Allreduce(&max_component_size_current_rank, &max_component_size, 1,
    ...
    timer.start_timer();
    get_int_ar_from_entity_ar<<<grid_size, block_size>>>(
        cc_distributed, cc_distributed_size, component_ar);
    timer.stop_timer();
    kernel_time = timer.get_spent_time();
    finalization_time += kernel_time;
    ...
    // List the cc counts for each process and calculate the displacements in
    // the final result
    start_time = MPI_Wtime();
    ...
    for (i = 1; i < total_rank; i++) {
        component_displacements[i] = component_displacements[i - 1] +
                                     (component_counts[i - 1] * total_columns);
    }
    end_time = MPI_Wtime();
    elapsed_time = end_time - start_time;
    finalization_time += elapsed_time;
    ...
    if (job_run == 0) {
        ...
        parallel_write(rank, total_rank, output_file_name, component_ar_host,
                       component_displacements, total_columns,
                       cc_distributed_size, &temp_file_write_time);
        cout << "Rank " << rank
    ...
    total_time = initialization_time + hashtable_build_time + join_time +
                 ...
                 deduplication_time + merge_time + finalization_time +
                 memory_clear_time;
    ...
    if (total_time == max_total_time) {
        ...
        printf(
            "# Input,# Process,# Iterations,# CC,# Nodes in largest WCC,Total "
            "Time,Join,Buffer "
            "preparation,Communication,Deduplication,Merge,Initialization,"
            "Hashtable,Finalization,Clear,File I/O\n");
        printf("%d,%d,%d,%lld,%lld,%.4lf,%.4lf,%.4lf,%.4lf,%.4lf,%.4lf,%.4lf,%."
               "4lf,"
               "%.4lf,%.4lf,%.4lf\n",
               output.input_rows, output.total_rank, output.iterations,
               output.output_size, output.output_size_secondary,
               output.total_time, output.join_time,
